...
static void check_validation_layer_support()
{
    ...
    for (const char* layer_name : validation_layers)
    {
        ...
        if (!found)
        {
            std::cerr << "Unsupported validation layer: " << layer_name << "\n";
            should_exit = true;
        }
    }
...
void Instance::destroy()
{
    if (enable_validation_layers)
    {
        DestroyDebugUtilsMessengerEXT(handle_, debug_handle_, nullptr);
    }
...
void Instance::setup_debug_messenger()
{
    ...
    VK_CHECK(CreateDebugUtilsMessengerEXT(handle_, &create_info, nullptr, &debug_handle_));
}
...
std::vector<VkPhysicalDevice> Instance::get_physical_devices() const
{
    ...
    vkEnumeratePhysicalDevices(handle_, &physical_device_count, physical_devices.data());
