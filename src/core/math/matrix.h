...
inline mat4f mat4_translation(const vec3f& v)
{
    // clang-format off
    return {
        1.0f, 0.0f, 0.0f, 0.0f,
...
inline mat4f mat4_scale(const vec3f& s)
{
    // clang-format off
    return mat4f::create({
        s.x , 0.0f, 0.0f, 0.0f,
...
inline mat4f mat4_rotation_x(float angle)
{
    float s = sinf(angle);
    float c = cosf(angle);
    // clang-format off
...
inline mat4f mat4_rotation_y(float angle)
{
    float s = sinf(angle);
    float c = cosf(angle);
    // clang-format off
...
inline mat4f mat4_rotation_z(float angle)
{
    float s = sinf(angle);
    float c = cosf(angle);
    // clang-format off
...
inline float mat4_determinant(const mat4f& m)
{
    float a = m[1][1] * m[2][2] * m[3][3] - m[1][1] * m[2][3] * m[3][2] - m[2][1] * m[1][2] * m[3][3]
        + m[2][1] * m[1][3] * m[3][2] + m[3][1] * m[1][2] * m[2][3] - m[3][1] * m[1][3] * m[2][2];
    float b = -m[1][0] * m[2][2] * m[3][3] + m[1][0] * m[2][3] * m[3][2] + m[2][0] * m[1][2] * m[3][3]
        - m[2][0] * m[1][3] * m[3][2] - m[3][0] * m[1][2] * m[2][3] + m[3][0] * m[1][3] * m[2][2];
    float c = m[1][0] * m[2][1] * m[3][3] - m[1][0] * m[2][3] * m[3][1] - m[2][0] * m[1][1] * m[3][3]
        + m[2][0] * m[1][3] * m[3][1] + m[3][0] * m[1][1] * m[2][3] - m[3][0] * m[1][3] * m[2][1];
    float d = -m[1][0] * m[2][1] * m[3][2] + m[1][0] * m[2][2] * m[3][1] + m[2][0] * m[1][1] * m[3][2]
        - m[2][0] * m[1][2] * m[3][1] - m[3][0] * m[1][1] * m[2][2] + m[3][0] * m[1][2] * m[2][1];
    float det = m[0][0] * a + m[0][1] * b + m[0][2] * c + m[0][3] * d;
    return det;
}

inline mat4f mat4_inverse(const mat4f& m)
{
    mat4f inv;
...
inline mat4f mat4_lookat(vec3f pos, vec3f target, vec3f up, mat4f* inv = nullptr)
{
...
inline mat4f mat4_perspective(float fov_y, float aspect_ratio, float _near, float _far, mat4f* inv = nullptr)
{
